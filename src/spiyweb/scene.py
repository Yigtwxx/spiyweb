...
the spring layout is seeded from a hash of the drawn id set (`rings.layout_seed`)
and why `hash()` is never used (it is salted per process). The hop-ring rule
itself lives in `rings.py`, numpy-free, so the terminal monitor draws from the
same rule; this module re-exports it.
"""
...
from spiyweb.rings import (
    RING_GHOST_GAP as RING_GHOST_GAP,
)
from spiyweb.rings import (
    RING_INNER as RING_INNER,
)
from spiyweb.rings import (
    RING_OUTER as RING_OUTER,
)
from spiyweb.rings import (
    RING_SINGLE as RING_SINGLE,
)
from spiyweb.rings import (
    hop_ring_layout as hop_ring_layout,
)
from spiyweb.rings import (
    layout_seed as layout_seed,
)
from spiyweb.rings import (
    ring_radii as ring_radii,
)
